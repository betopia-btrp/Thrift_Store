              ...
              <div key={order.order_id} className="bg-white border border-gray-200 rounded-xl p-4">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1">
                      {order.status_history.map((h: any, i: number) => (
                        <div key={i} className="flex items-center gap-1 flex-shrink-0">
                          ...
                          <span className="text-xs text-gray-500 capitalize">{h.new_status}</span>
                ...
                {order.status === 'confirmed' || order.status === 'dispatched' ? (
                  <p className="text-xs text-amber-600 mt-2 bg-amber-50 px-2 py-1 rounded">
                    Delivery is arranged directly between you and the seller.
                  </p>
                ) : null}
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  {order.review && (
                    <span className="text-xs text-green-600 border border-green-200 px-3 py-1 rounded-lg">
                      ✓ Reviewed
                    </span>
                  )}
                </div>
              ...
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    ...
                    className={`text-2xl transition ${star <= reviewForm.rating ? 'text-amber-400' : 'text-gray-300'}`}
                  >
                    ★
                  </button>
                ))}
