...
class OrderController extends Controller
{
    ...
    public function store(StoreOrderRequest $request)
    {
        ...
        // Check if buyer already has a pending/completed order for this product
        $existingOrder = Order::where('buyer_id', $buyer->user_id)
            ->where('product_id', $request->product_id)
            ->whereIn('status', ['pending', 'accepted', 'completed'])
            ->first();

        if ($existingOrder) {
            return ApiResponse::error('You already have an order for this listing', 422);
        }
...
public function sellerStats()
{
    ...
    public function buyerOrders()
    {
        ...
        $orders = Order::with([
            ...
            ->where('buyer_id', $buyer->user_id) // already scoped — good
    ...
    // In show() — verify buyer OR seller owns this order
    public function show(string $id)
    {
        ...
        // Strict ownership check — no other user can see this order
        if (! $isBuyer && ! $isSeller && $user->role !== 'admin') {
            return ApiResponse::error('You do not have permission to view this order', 403);
        }
